...
func (c *PinsAPIController) Routes() Routes {
	return Routes{
		...
		"DeletePinByRequestId": Route{
			strings.ToUpper("Delete"),
			"/pins/{requestid}",
			c.DeletePinByRequestId,
		},
		"GetPinByRequestId": Route{
			strings.ToUpper("Get"),
			"/pins/{requestid}",
			c.GetPinByRequestId,
		},
...
func (c *PinsAPIController) AddPin(w http.ResponseWriter, r *http.Request) {
	...
	if err := AssertPinConstraints(pinParam); err != nil {
		c.errorHandler(w, r, err, nil)
		return
	}
...
func (c *PinsAPIController) GetPins(w http.ResponseWriter, r *http.Request) {
	...
	var matchParam TextMatchingStrategy
	if query.Has("match") {
		param := TextMatchingStrategy(query.Get("match"))
	...
	if query.Has("status") {
		...
		statusParam = make([]Status, 0, len(paramSplits))
		for _, param := range paramSplits {
			paramEnum, err := NewStatusFromValue(param)
			if err != nil {
	...
	var beforeParam time.Time
	if query.Has("before"){
		param, err := parseTime(query.Get("before"))
		if err != nil {
			c.errorHandler(w, r, &ParsingError{Param: "before", Err: err}, nil)
			return
		}
	...
	var afterParam time.Time
	if query.Has("after"){
		param, err := parseTime(query.Get("after"))
		if err != nil {
			c.errorHandler(w, r, &ParsingError{Param: "after", Err: err}, nil)
			return
		}
	...
	var metaParam map[string]string
	if query.Has("meta") {
		metaParam = make(map[string]string)
		err := json.Unmarshal([]byte(query.Get("meta")), &metaParam)
		if err != nil {
			c.errorHandler(w, r, &ParsingError{Param: "meta", Err: err}, nil)
			return
		}
	} else {
	}
