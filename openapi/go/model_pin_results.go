...
// AssertPinResultsConstraints checks if the values respects the defined constraints
func AssertPinResultsConstraints(obj PinResults) error {
	if obj.Count < 0 {
		return &ParsingError{Param: "Count", Err: errors.New(errMsgMinValueConstraint)}
	}
	for _, el := range obj.Results {
		if err := AssertPinStatusConstraints(el); err != nil {
			return err
		}
	}
	return nil
