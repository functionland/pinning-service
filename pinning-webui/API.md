...
**Errors:** `400` invalid CAR (junk/truncated, hash mismatch, 0 or >1 roots, root block absent, incomplete DAG, unsupported codec, oversized block, too many blocks, or a dag-cbor block nested beyond `DAG_IMPORT_MAX_DAG_DEPTH`) — the message carries the specific reason; `402` insufficient credits — three layers, all evaluated before any data is imported: (1) the standard gate (suspended, or over free tier with zero balance), (2) a zero-balance user may not cross the 500 MB free-tier boundary, (3) the balance must cover the **projected post-import storage** over the free tier for `DAG_IMPORT_MIN_BALANCE_DAYS` (default 30 — one month) at `FULA_PER_GB_MONTH` (default 3); e.g. importing 800 MB at the free-tier boundary requires ≥ 2.4 FULA. Imports fitting inside the free tier require no balance; `413` CAR larger than `DAG_IMPORT_MAX_CAR_BYTES` (default 800 MB); `429` too many concurrent imports — either the global cap (`DAG_IMPORT_MAX_CONCURRENT`, default 5) or the per-user cap (`DAG_IMPORT_MAX_CONCURRENT_PER_USER`, default 1).
